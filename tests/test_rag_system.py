...
from app.modules.RAG import initialize_rag_system
import requests
import time
...
def test_rerank_results():
    ...
    for query in test_queries:
        ...
        # 测试重排序检索
        if settings.RAG_CONFIG["reranker"].get("enable", False):
            logger.info("\n--- 重排序结果 ---")
...
def test_ollama_service():
    ...
    """
    try:
        response = requests.get(f"{settings.SERVICE_CONFIG['ollama_host']}/api/tags", timeout=10)
        ...
        logger.error(f"❌ Ollama服务连接异常: {str(e)}")
        return False

def test_modelscope_api():
    """
    @brief 测试ModelScope API连接
    """
    try:
        api_key = settings.MODELSCOPE_API_KEY
        if not api_key:
            logger.warning("ModelScope API密钥未设置")
            return False
        ...
        try:
            client = OpenAI(
                api_key=api_key,
                base_url=settings.SERVICE_CONFIG['modelscope_base_url']
            )
            
            # 简单测试API可用性
            models = client.models.list()
            logger.info("✅ ModelScope API连接正常")
            return True
        except Exception as e:
        ...
        logger.error(f"❌ ModelScope API连接异常: {str(e)}")
        return False
...
def test_embedding_generation():
    ...
    """
    try:
        from app.modules.RAG.embeddings import EmbeddingModel
...
if __name__ == "__main__":
    ...
    # 根据配置检查相应的基础服务
    embedding_config = settings.RAG_CONFIG["embeddings"]
    ...
    # 检查重排序服务（如果启用）
    reranker_config = settings.RAG_CONFIG["reranker"]
    ...
    # 检查摘要服务
    summarizer_config = settings.RAG_CONFIG["summarizer"]
